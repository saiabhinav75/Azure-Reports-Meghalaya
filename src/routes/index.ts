...
ReportRouter.use(express.raw({ type: 'application/octet-stream', limit: '50mb' }));
...
ReportRouter.post('/get-report',(req,res,next)=>{
  ...
  const base64Audio:string = req.body.audioBufferBase64 as string;
...
// const token = authHeader && authHeader.split(' ')[1]
    ...
    try{
      const report = await main(buffer,subscriptionKey,serviceRegion,reference_text,studentClass)as Report;
      if(report){
        const {data:mera_data,error:tera_error}=await supabaseClient.from("RCA_Results").select("id,partB_score").eq("student_id",student_id).eq("assessment_id",assessment_id)
        if(tera_error){
        ...
        if(mera_data && mera_data.length>0){
          ...
          const {data,error} = await supabaseClient.from("RCA_Results").update([{partA_report:report,partA_score:report.partA_score.score,total:(report.partA_score.score + mera_data[0].partB_score)/2,grade:final_bucket}]).eq("id",mera_data[0].id);
          if(error){
          ...
          const {data,error} = await supabaseClient.from("RCA_Results").insert([{partA_report:report,partA_score:report.partA_score.score, assessment_id,student_id}]);
          console.log(data)
        }
